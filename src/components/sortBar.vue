<template>
    <div  id="sortBar" class="flex_auto p-r_4">
        ...
        <ul class="flex flex_column flex_row:md font_bold font_n2 font_ui ul_none">
            <li class="flex_shrink" v-for="(type,index) in sortTypes" :key="'sortbutton_'+index" :class="{'is-active': selectedSort == type}" v-on:click="selectedSortChange(type)">
                <span
...
<script>
    export default {
        name:'sortBar',
        props:{
            sortTypes:{type:Array, default:()=>["Date","Activity","Product"]},
